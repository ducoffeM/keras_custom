...
import numpy as np
import keras
from keras_custom.layers.onnx import PlusConstant, MulConstant
...
def func_layer(layer, input_shape):
    ...
    try:
        ...
        np.testing.assert_almost_all_equal(output_after_training, output_after_export, err_msg='corrupted weights')
    except:
        if os.path.isfile(filename):
            # delete filename
            os.remove(filename)
        raise ValueError('error when deserializing the model')
    """
