...
def relu_prime(
    inputs: Tensor, negative_slope: float = 0.0, threshold: float = 0.0, max_value: Union[None, float] = None
) -> Tensor:
    """
...
def relu6_prime(inputs: Tensor, negative_slope: float = 0.0, threshold: float = 0.0) -> Tensor:
    ...
    """
    return relu_prime(inputs, negative_slope=negative_slope, threshold=threshold, max_value=6.0)
...
def hard_silu_prime(inputs: Tensor) -> Tensor:
    ...
    mask_greater_than_minus_3: Tensor = K.relu(K.sign(inputs + 3 + keras.backend.epsilon()))  # 1 if inputs >= -3
    mask_lower_than_3: Tensor = K.relu(K.sign(3 + keras.backend.epsilon() - inputs))  # 1 if inputs <=3

    return (1 - mask_lower_than_3) + (inputs / 3.0 + 0.5) * mask_lower_than_3 * mask_greater_than_minus_3
