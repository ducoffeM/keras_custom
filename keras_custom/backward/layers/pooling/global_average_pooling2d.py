...
class BackwardGlobalAveragePooling2D(BackwardLinearLayer):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        if self.layer.data_format == "channels_first":
            w_in, h_in = self.layer.input.shape[-2:]
            if self.layer.keepdims:
                output= K.repeat(K.repeat(inputs, w_in, -2), h_in, -1) / (w_in * h_in)
            else:
                output= K.repeat(K.repeat(K.expand_dims(K.expand_dims(inputs, -1), -1), w_in, -2), h_in, -1) / (
                    w_in * h_in
                )
        else:
            w_in, h_in = self.layer.input.shape[1:3]
            if self.layer.keepdims:
                output= K.repeat(K.repeat(inputs, w_in, 1), h_in, 2) / (w_in * h_in)
            else:
                output= K.repeat(K.repeat(K.expand_dims(K.expand_dims(inputs, 1), 1), w_in, 1), h_in, 2) / (w_in * h_in)
...
def get_backward_GlobalAveragePooling2D(layer: GlobalAveragePooling2D, use_bias=True) -> Layer:
    ...
    backward_layer = get_backward_GlobalAveragePooling2D(average_layer, use_bias=True)
    output = backward_layer(input_tensor)
    """
